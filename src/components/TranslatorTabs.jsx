...
const TranslatorTabs = ({
  ...
  const getOrderedTranslators = () => {
    if (!translators || !enabledTranslators || Object.keys(translators).length === 0) {
      return []
    }
    
    // 过滤出启用的翻译器
    const filtered = Object.entries(translators)
      .filter(([key]) => enabledTranslators.includes(key))
  ...
  const handleTranslatorClick = (key, name, needsConfig, isConfigured) => {
    ...
    if (needsConfig && !isConfigured) {
      window.utools && window.utools.showNotification 
        ? window.utools.showNotification(`请先在设置中配置${name}的API密钥`)
        : alert(`请先在设置中配置${name}的API密钥`);
      return;
    }
      ...
      case 'glm':
        icon = (
          <svg viewBox="0 0 1024 1024" width="24" height="24">
            <path d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64z" fill="#1890ff"/>
            ...
            <path d="M512 384c-70.7 0-128 57.3-128 128s57.3 128 128 128 128-57.3 128-128-57.3-128-128-128z" fill="#ffffff"/>
          </svg>
        )
        break
      case 'caiyun':
        icon = (
          <svg viewBox="0 0 1024 1024" width="24" height="24">
            <path d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64z" fill="#1890ff"></path>
            <path d="M713.5 378c-30.9-30.9-79.4-37.9-118.2-17-14.3 7.7-25.5 18.9-33.4 31.9-8-12.9-19.1-24.2-33.4-31.9-38.7-20.9-87.2-13.9-118.2 17-35.3 35.3-38.8 90.5-10.6 130.2L512 660.8l112.2-152.6c28.3-39.7 24.8-95-10.7-130.2z" fill="#ffffff"></path>
          </svg>
        )
        break
      case 'deepl':
    ...
    return (
      ...
      >
        ...
        {needsConfig && !isConfigured && (
          <div className="config-indicator">!</div>
        )}
      </div>
    )
  }
  ...
  return (
    <div className="translator-tabs">
      {orderedTranslators.length > 0 ? (
        ...
        // 如果没有可用的翻译器，显示默认选项
        <div className="translator-tab auto-detect no-translators">
          <div className="translator-icon">
            <svg viewBox="0 0 1024 1024" width="24" height="24">
              <path d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64z m0 820c-205.4 0-372-166.6-372-372s166.6-372 372-372 372 166.6 372 372-166.6 372-372 372z" fill="#1890ff"></path>
              <path d="M512 336c-97.2 0-176 78.8-176 176s78.8 176 176 176 176-78.8 176-176-78.8-176-176-176z" fill="#1890ff"></path>
            </svg>
          </div>
          <div className="translator-name">
            请配置翻译服务
